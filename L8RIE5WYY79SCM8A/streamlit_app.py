...
# Stream generator for word-by-word display
def stream_generator():
    """Generate streaming response word by word."""
    if 'response_text' in st.session_state and st.session_state.response_text:
        words = st.session_state.response_text.split()
        for word in words:
            yield word + " "

# --- Main App ---
st.title(":material/chat: Optimized Streaming Chatbot")
st.success("AI-powered chatbot with model selection, streaming responses, and conversation modes")
...
if prompt := st.chat_input("Type your message..."):
    ...
    # Auto-generate and display summary if enough messages
    if len(st.session_state.messages) >= 10 and len(st.session_state.messages) % 4 == 0:
        st.divider()
        with st.container():
            st.subheader(":books: Conversation Summary")
...
st.divider()
st.caption("Day 12: Optimized Streaming Responses | 30 Days of AI")